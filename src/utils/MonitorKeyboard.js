...
class MonitorKeyboard {
  ...
  onResize = () => {
    ...
    if (this.originalHeight - resizeHeight > 50) {
      this.show && this.show("Android系统: 软键盘弹出");
    } else {
      this.hidden && this.hidden("Android系统: 软键盘收起");
    }
  };

  // iOS获取焦点
  onFocusin = () => {
    this.show && this.show("iOS系统:软键盘弹出");
  };

  // iOS失去焦点
  onFocusout = () => {
    this.hidden && this.hidden("iOS系统:软键盘收起");
  };
   ...
   * @param {Function} fn 虚拟键盘弹出时触发
   */
  onShow = (fn) => {
    this.show = fn || console.log;
  };

  /**
   * @function  onHidden 传递一个回调函数
   * @param {Function} fn 虚拟键盘隐藏时触发
   */
  onHidden(fn) {
    this.hidden = fn || console.log;
  }
}
